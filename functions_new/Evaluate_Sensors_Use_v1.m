...
%
%%
function [sensors_perc_use_over_subjects_cell, sensors_perc_use_over_speeds_cell, sensors_perc_use_overall_cell] = Evaluate_Sensors_Use_v1(subjs_in)
...
for i=1:n_subjs
    ...
    for j=1:n_speeds
        ...
        subji_speedj_sensors_used = subjs_in(i).data.(speed_list{j}).sensor_reduced.sensors_tokeep;
        for k=1:6
            %             sensor1_use(i,j) = sum(ismember(subji_speedj_sensors_used,k));
            command_to_launch = ['sensor',num2str(k),...
                '_use(i,j) = sum(ismember(subji_speedj_sensors_used,',num2str(k),'));'];
