...
%%
function [subjs_out, corr_out] = Run_Correlation_PC_Spiro_v2(subjs_in,domain_analysis,pc_selected,spiro_signal_integral)
...
for i=1:length(subjs_out)
    ...
    for j=1:n_speeds
        ...
        if(time_domain_analysis_flag)
            ...
            % Compute nPSD first
            srate = 250;
            [P_norm_pcsig, freq_pcsig, P_pcsig,~] = Perform_PSD_v2(pc_sig,srate);
        ...
        % Save data ouput
        new_field_name = ['pc',pc_selected,'_',spiro_signal_integral,'_',domain_analysis];
        subjs_out(i).data.(speed_list{j}).(new_field_name).R = R(i,j);
        subjs_out(i).data.(speed_list{j}).(new_field_name).p = p(i,j);
        subjs_out(i).data.(speed_list{j}).(new_field_name).variance_expl_pc1 = var_expl(i,j);
