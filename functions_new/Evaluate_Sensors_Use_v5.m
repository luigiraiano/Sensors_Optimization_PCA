...
% 4-    average of the number of sensors used for each subject at each
%       speed
%
% Input parameters
% 1-    subjects structure, accroding to main_ElabNoIMU_SensorsPlacementOptimization_PCA_v3
% 2-    flag to choose senros to be used to get tables. It can assume the
%       folloing values:
%       a- 'clean_sensors' -> use sensors obtained after step 5
%       b- 'non_redundant_sensors' -> use sensors obtained after step 7
%
%%
function [sensors_perc_use_over_subjects_cell, sensors_perc_use_over_speeds_cell, sensors_perc_use_overall_cell, n_sensors_used] = Evaluate_Sensors_Use_v5(subjs_in,sensors_tb_used)
% check the preasence of the sensor within each subject for each speed.
% si o no. Perc = si/si+no

use_non_redundant = true;
if(strcmp(sensors_tb_used,'non_redundant_sensors'))
    use_non_redundant = true;
elseif(strcmp(sensors_tb_used,'clean_sensors'))
    use_non_redundant = false;
end
...
for i=1:n_subjs
    ...
    for j=1:n_speeds
        ...
        clean_sensors = [];
        clean_sensors = subjs_in(i).data.(speed_list{j}).clean_sensors.sensors_tokeep;
        noisy_sensors_discarded = subjs_in(i).data.(speed_list{j}).clean_sensors.all_sensors_discarded;
        ...
        if(use_non_redundant)
            ...
            % use only non redundant sensors
            ...
            % Get number of sensors used for subject i and speed j
            %             n_sensors_used.subjs_and_speeds(i,j) = length(subjs_in(i).data.(speed_list{j}).sensors_reduced.sensors_tokeep);
            n_sensors_used.subjs_and_speeds(i,j) = length(non_redundant_sensors);
            
        else
            
            % use all sensors except the noisy ones
            for k=1:length(clean_sensors) % loop on sensors
                command_to_launch = [];
                command_to_launch = ['sensor',num2str(clean_sensors(k)),...
                    '_use(i,j) = sum(ismember(clean_sensors(k),all_sensors));'];
                eval(command_to_launch);
            end % end for k (all sensors)
            
            k = 1;
            for k=1:length(noisy_sensors_discarded) % loop on sensors
                command_to_launch = [];
                command_to_launch = ['sensor',num2str(noisy_sensors_discarded(k)),...
                    '_use(i,j) = 0;'];
                eval(command_to_launch);
            end % end for k (all sensors)
            
            % Get number of sensors used for subject i and speed j
            n_sensors_used.subjs_and_speeds(i,j) = length(clean_sensors);
...
sensors_perc_use_over_subjects = round(sensors_perc_use_over_subjects,4);
...
sensors_perc_use_over_speeds = round(sensors_perc_use_over_speeds,4);
...
check = round(sensors_perc_use_overall_a - sensors_perc_use_overall_b,4);

sensors_perc_use_overall_cell = num2cell(sensors_perc_use_overall_b);

n_sensors_used.mean_along_subjects_over_speed = mean(n_sensors_used.subjs_and_speeds,1);
n_sensors_used.mean_along_speeds_over_subjects = mean(n_sensors_used.subjs_and_speeds,2);
n_sensors_used.mean_use = mean(mean(n_sensors_used.subjs_and_speeds));
end % end function