...
%
% Segnale ritagliato (60 s) e bp filtered
...
%%
function subj_struct = SensorsOptimizationPCA_LoadDataNoIMU_v2(subj_dir_path)
...
for i=1:length(subj_data)
    ...
    subj_info = subj_tmp_file_name_info{1,1}{1};
    subj_name = subj_info(1:2);
    subj_speed = subj_info(3:4);
    subj_id = subj_tmp_file_name_info{1,1}{5};
    
    subj_struct.id = subj_id;
    subj_struct.name = subj_name;
    ...
    %% Already BP filtered data
%     subj_struct.data.(['speed_',subj_speed]).bpflt.quattrosegnali = quattrosegnali; % [V]
%     subj_struct.data.(['speed_',subj_speed]).bpflt.f_SG = f_SG; % [Hz]
%     subj_struct.data.(['speed_',subj_speed]).bpflt.f_SGBPM = f_SGBPM; % [Hz]
    %%
