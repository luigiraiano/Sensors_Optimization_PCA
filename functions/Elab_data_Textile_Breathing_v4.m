function subj = Elab_data_Textile_Breathing_v4(subj_dir_path,subjs_dir_name,explained_perc_thres,perce_sensors_weigth_thres,corr_thresh)
...
for i=1:length(subj_data)
    ...
    %% Get the weight of each sensor in PCA that explain the explained_perc_thres % variability of the orignal data
    ...
    %% Correlation between each component and all sensors (not provided in output)
    subj.(speed_string).dim_red.info6_bis = "/******** Dimensionality Reduction - correlation between all sensors in order to find out eventual copies ********/";
    % data cleaned removing the artefact of the torso rotation
    
    % Correlazioni tra sensori
    R_sens_noTorso = []; p_noTorso = [];
    for i=1:size(piezo_maglia_noTorso,2) % loop sui sensori
        for j=1:size(piezo_maglia_noTorso,2) % loop sui sensori
            [R_sens_noTorso(i,j),p_noTorso(i,j)] = corr(piezo_maglia_noTorso(:,i), piezo_maglia_noTorso(:,j));
        end % end for j
    end % end for i
    subj.(speed_string).dim_red.R_sens_noTorso = R_sens_noTorso;
    subj.(speed_string).dim_red.p_noTorso = p_noTorso;
    
    % Correlazioni tra sensori
    R_sens_ICArec = []; p_ICArec = [];
    for i=1:size(piezo_maglia_ICArec,2) % loop sui sensori
        for j=1:size(piezo_maglia_ICArec,2) % loop sui sensori
            [R_sens_ICArec(i,j),p_ICArec(i,j)] = corr(piezo_maglia_ICArec(:,i), piezo_maglia_ICArec(:,j));
            
        end % end for j
    end % end for i
    subj.(speed_string).dim_red.R_sens_ICArec = R_sens_ICArec;
    subj.(speed_string).dim_red.p_ICArec = p_ICArec;
    %% Check if sensors are highly correlated and refres to pearson's coeff. Threshold is equal to: corr_thresh
    % no torso corr.
    subj.(speed_string).dim_red.info8 = "/******** check redundancy in sensors and in case remove ********/";

    all_chans = 1:6;
    
    R_noTorso_uppertriangle = triu(R_sens_noTorso);
    comp_sens_noTorso = [];
    for i=1:size(piezo_maglia_noTorso,2) % loop sui sensori
        for j=1:size(piezo_maglia_noTorso,2) % loop sui sensori
            if(R_noTorso_uppertriangle(i,j) >= corr_thresh && (i~=j) )
                % check which of the two sensors has a higher weight in PCA
                if(ave_weights_perc_ICArec_sensors(i) >= ave_weights_perc_ICArec_sensors(j))
                    comp_sens_noTorso.(['sens_',num2str(i),'_vs_sens_',num2str(j),'_kept']) = i;
                else
                    comp_sens_noTorso.(['sens_',num2str(i),'_vs_sens_',num2str(j),'_kept']) = j;
                end % end if-else
            end % end if
        end % end for j
    end % end for i
    
    if(~isempty(comp_sens_noTorso))
        subj.(speed_string).dim_red.corr_sensors = comp_sens_noTorso;
        
        chan_excluded_noTorso_corr_tmp = [];
        chan_excluded_noTorso_corr = [];
        fields = [];
        fields = fieldnames(comp_sens_noTorso);
        l = [];
        l = length(fields);
        for i=1:l
            chan_excluded_noTorso_corr_tmp(i) = comp_sens_noTorso.(fields{i});
        end % end for i
        chan_excluded_noTorso_corr = unique(chan_excluded_noTorso_corr_tmp,'first');
        
        subj.(speed_string).dim_red.n_chan_excluded_noTorso_corr = length(chan_excluded_noTorso_corr);
        subj.(speed_string).dim_red.chan_excluded_noTorso_corr = chan_excluded_noTorso_corr;
        
        % chans included
        chans_included_noTorso_corr = all_chans;
        for i=1:length(chan_excluded_noTorso_corr)
            find_noTorso(i) = find(chan_excluded_noTorso_corr(i) == all_chans);
        end % end for i

        chans_included_noTorso_corr(find_noTorso) = [];
        
        subj.(speed_string).dim_red.chan_included_noTorso_corr = chans_included_noTorso_corr;
    else
        subj.(speed_string).dim_red.corr_sensors = [];
        subj.(speed_string).dim_red.n_chan_excluded_noTorso_corr = 0;
        subj.(speed_string).dim_red.chan_excluded_noTorso_corr = [];
        subj.(speed_string).dim_red.chan_included_noTorso_corr = all_chans;
    end
    
    % ICA rec
    R_ICArec_uppertriangle = triu(R_sens_ICArec);
    comp_sens_ICArec = [];
    for i=1:size(piezo_maglia_ICArec,2) % loop sui sensori
        for j=1:size(piezo_maglia_ICArec,2) % loop sui sensori
            if(R_ICArec_uppertriangle(i,j) >= corr_thresh && (i~=j) )
                % check which of the two sensors has a higher weight in PCA
                if(ave_weights_perc_ICArec_sensors(i) >= ave_weights_perc_ICArec_sensors(j))
                    comp_sens_ICArec.(['sens_',num2str(i),'_vs_sens_',num2str(j),'_kept']) = i;
                else
                    comp_sens_ICArec.(['sens_',num2str(i),'_vs_sens_',num2str(j),'_kept']) = j;
                end % end if-else
            end % end if
        end % end for j
    end % end for i
    
    if(~isempty(comp_sens_ICArec))
        subj.(speed_string).dim_red.corr_sensors = comp_sens_ICArec;
        
        chan_excluded_ICArec_corr_tmp = [];
        chan_excluded_ICArec_corr = [];
        fields = [];
        fields = fieldnames(comp_sens_ICArec);
        l = [];
        l = length(fields);
        for i=1:l
            chan_excluded_ICArec_corr_tmp(i) = comp_sens_ICArec.(fields{i});
        end % end for i
        chan_excluded_ICArec_corr = unique(chan_excluded_ICArec_corr_tmp,'first');
        
        subj.(speed_string).dim_red.n_chan_excluded_ICArec_corr = length(chan_excluded_ICArec_corr);
        subj.(speed_string).dim_red.chan_excluded_ICArec_corr = chan_excluded_ICArec_corr;
        
        % chans included
        chans_included_ICArec_corr = all_chans;
        for i=1:length(chan_excluded_ICArec_corr)
            find_ICArec(i) = find(chan_excluded_ICArec_corr(i) == all_chans);
        end % end for i
        

        chans_included_ICArec_corr(find_ICArec) = [];
        subj.(speed_string).dim_red.chan_included_ICArec_corr = chans_included_ICArec_corr;
        
        
    else
        subj.(speed_string).dim_red.corr_sensors = [];
        subj.(speed_string).dim_red.n_chan_excluded_ICArec_corr = 0;
        subj.(speed_string).dim_red.chan_excluded_ICArec_corr = [];
        subj.(speed_string).dim_red.chan_included_ICArec_corr = all_chans;
    end % end if
