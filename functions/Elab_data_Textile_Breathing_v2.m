function subj = Elab_data_Textile_Breathing_v2(subj_dir_path,subjs_dir_name)
...
for i=1:length(subj_data)
    ...
    %% Remove possible torax rotation artifacts
    % Combine all textile signal with gyro y signal (in freq. domain)
    freq_torso_rotation = [];
    for i=1:size(piezo_maglia,2)
        [piezo_maglia_noTorax(:,i), freq_torso_rotation(i)] = Remove_Gyro_Artifact_v2(piezo_maglia(:,i)',gyro(:,2),srate);
    end % end for i
    
    subj.(speed_string).data.piezo_maglia_noTorax = piezo_maglia_noTorax;
    subj.(speed_string).data.freq_torso_rotation = freq_torso_rotation;
    ...
    % figure;
    discard_channels = [];
    for i=1:size(piezo_maglia_noTorax,2)
        [f_sigs_raw(i),discard_channels(i)] = PSD_v3(piezo_maglia_noTorax(:,i)',srate,'discard_channels','donotplot');
    end % end for i
    %% Remove Signal too noisy
    chans_ok = [];
    chans_ok = find(discard_channels==0);
    piezo_maglia_new = piezo_maglia_noTorax(:,chans_ok);
    ...
    subj.(speed_string).data.f_breathing_textile_ave = Get_Mean_Breathing_Rate_v1(piezo_maglia_new_rec_ave,srate,0.6);
